	...
	protected int peek(InetAddress sender) throws IOException {
		if (isNativeConnected) {
			...
			netImpl.recvConnectedDatagram(fd, pack, pack.getData(),
					pack.getOffset(), pack.getLength(), receiveTimeout, true); // peek
	...
	public void receive(DatagramPacket pack) throws java.io.IOException {
		try {
			if (isNativeConnected) {
				...
				netImpl.receiveDatagram(fd, pack, pack.getData(),
						pack.getOffset(), pack.getLength(), receiveTimeout,
						false);
			}
	...
	public void send(DatagramPacket packet) throws IOException {
		...
		if (isNativeConnected) {
			netImpl.sendConnectedDatagram(fd, packet.getData(),
					packet.getOffset(), packet.getLength(), bindToDevice);
		} else {
			// sendDatagramImpl2
	...
	public void setOption(int optID, Object val) throws SocketException {
		...
		if (optID == SocketOptions.SO_TIMEOUT) {
			...
			if (optID == SocketOptions.IP_MULTICAST_IF
				...
				// if (NetUtil.bytesToInt(inet.getAddress(), 0) == 0 ||
